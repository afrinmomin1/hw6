...
{
    if (static_cast<double>(countUsed_) / CAPACITIES[mIndex_] >= resizeAlpha_) {
        resize();
    }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    countActive_ = 0;
    countUsed_ = 0;
    for (HashItem* ptr : oldTable) {  // explicit pointer type
        if (ptr && !ptr->deleted) {
            insert(ptr->item);
        }
        delete ptr;
